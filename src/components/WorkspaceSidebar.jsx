...
const buildWorkspacePath = (workspaceId, path) => `/workspace/${workspaceId}${path}`;
    ...
    <aside className="sidebar">
      <div className="brand">
        ...
        <div>
          ...
          <p className="brand-subtitle">ID: {workspaceId || "-"}</p>
        </div>
      </div>
      <div className="workspace-status">
        <a className="nav-link" href={buildHashHref("/")}>Back to Platform</a>
      </div>
      <div className="workspace-status">
          ...
          <div className={`nav-section nav-${section.theme}`} key={section.title}>
            ...
            <div className="nav-subsections">
              {section.children.map((group) => {
                ...
                const lockTitle = isLocked ? "Available after Create Recipe" : isReadOnly ? "Read-only in current status" : "";
