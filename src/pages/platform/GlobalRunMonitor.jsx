...
const tabs = ["running", "queued", "completed"];

export default function GlobalRunMonitor() {
  const [activeTab, setActiveTab] = useState("running");
  const runs = listGlobalRuns().filter((run) => run.status === activeTab);
        ...
        <div className="table">
          ...
          {runs.map((run) => (
            <a
              ...
              href={buildHashHref(`/workspace/${run.modelID}/run-monitor/control`)}
            >
